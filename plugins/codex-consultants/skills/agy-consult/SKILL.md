...
Use `$agy-consult` when you want Antigravity to challenge Codex's current understanding. `$agy-consultant` remains available as the legacy name.
...
Use the bundled `scripts/agy_consult.py` wrapper through the installed `codex-agy-consult` launcher, or directly from the `agy-consultant` skill directory when the plugin is installed without the manual launcher. Choose `--phase plan` before implementation or `--phase diff` after implementation, and include only relevant files with repeated `--path` arguments.
